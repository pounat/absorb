...
void showEqualizerSheet(BuildContext context, Color accent, {String? itemId, String? itemTitle}) {
  showModalBottomSheet(
    ...
    builder: (ctx) => DraggableScrollableSheet(
      ...
      snap: true,
      maxChildSize: 0.92,
      builder: (ctx, sc) => _EqualizerSheetContent(accent: accent, scrollController: sc, openedForItemId: itemId, openedForItemTitle: itemTitle),
    ),
  );
}
...
class _EqualizerSheetContent extends StatefulWidget {
  ...
  const _EqualizerSheetContent({required this.accent, required this.scrollController, this.openedForItemId, this.openedForItemTitle});
...
class _EqualizerSheetContentState extends State<_EqualizerSheetContent> {
  ...
  void _syncPreviewMode() {
    final shouldPreview = _eq.perItem && widget.openedForItemId != null && widget.openedForItemId != _eq.currentItemId;
  ...
  String _presetLabel(AppLocalizations l, String name) {
    switch (name) {
      case 'flat':
        return l.equalizerPresetFlat;
      case 'voice boost':
        return l.equalizerPresetVoiceBoost;
      case 'bass boost':
        return l.equalizerPresetBassBoost;
      case 'treble boost':
        return l.equalizerPresetTrebleBoost;
      case 'podcast':
        return l.equalizerPresetPodcast;
      case 'audiobook':
        return l.equalizerPresetAudiobook;
      case 'reduce noise':
        return l.equalizerPresetReduceNoise;
      case 'loudness':
        return l.equalizerPresetLoudness;
      default:
  ...
  Widget _buildScopeBanner(ColorScheme cs, TextTheme tt, Color accent, AppLocalizations l) {
    ...
    final message = widget.openedForItemTitle != null ? l.equalizerEditingSavedNamed(widget.openedForItemTitle!) : l.equalizerEditingSavedGeneric;
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(message, style: tt.labelSmall?.copyWith(color: cs.onSurface.withValues(alpha: 0.75), height: 1.3)),
            ),
          ],
        ),
      ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Text(
                  l.audioEnhancements,
                  style: tt.titleMedium?.copyWith(fontWeight: FontWeight.w600, color: cs.onSurface),
                ),
                const Spacer(),
                // Master toggle
                Transform.scale(
                  scale: 0.8,
                  child: Switch(value: _vEnabled, activeTrackColor: accent, onChanged: (v) => _setEnabled(v)),
                ),
              ],
            ),
          ),
          ...
          Padding(
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Icon(Icons.library_music_rounded, size: 16, color: _eq.perItem ? accent : cs.onSurfaceVariant),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        l.equalizerPerBookEq,
                        style: tt.labelMedium?.copyWith(color: _eq.perItem ? accent : cs.onSurface.withValues(alpha: 0.75), fontWeight: FontWeight.w600),
                      ),
                    ),
                    Transform.scale(
                      scale: 0.7,
                      child: Switch(value: _eq.perItem, activeTrackColor: accent, onChanged: (v) => _eq.setPerItem(v)),
                    ),
                  ],
                ),
              ),
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                Text(
                  l.presets,
                  style: tt.labelSmall?.copyWith(color: cs.onSurface.withValues(alpha: 0.3), letterSpacing: 1.5, fontWeight: FontWeight.w600),
                ),
                    ...
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: isActive ? accent.withValues(alpha: 0.5) : cs.onSurface.withValues(alpha: 0.1)),
                        ),
                        child: Text(
                          _presetLabel(l, name),
                          style: TextStyle(
                            color: _vEnabled ? (isActive ? accent : cs.onSurface.withValues(alpha: 0.7)) : cs.onSurface.withValues(alpha: 0.24),
                            fontSize: 12,
                ...
                // Bass Boost
                _EffectRow(icon: Icons.speaker_rounded, label: l.bassBoost, value: _vBass, accent: accent, enabled: true, onChanged: (v) => _setBass(v)),
                const SizedBox(height: 8),

                // Virtualizer (Android only - no iOS equivalent)
                if (Platform.isAndroid) ...[
                  _EffectRow(icon: Icons.surround_sound_rounded, label: l.surround, value: _vVirt, accent: accent, enabled: true, onChanged: (v) => _setVirt(v)),
                  const SizedBox(height: 8),
                ],

                // Loudness
                _EffectRow(icon: Icons.volume_up_rounded, label: l.loudness, value: _vLoud, accent: accent, enabled: true, onChanged: (v) => _setLoud(v)),
                const SizedBox(height: 8),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.headphones_rounded, size: 18, color: _vMono ? accent.withValues(alpha: 0.7) : cs.onSurface.withValues(alpha: 0.2)),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Text(
                          l.monoAudio,
                          style: TextStyle(color: _vMono ? cs.onSurface.withValues(alpha: 0.7) : cs.onSurface.withValues(alpha: 0.24), fontSize: 12, fontWeight: FontWeight.w500),
                        ),
                      ),
                      Transform.scale(
                        scale: 0.8,
                        child: Switch(value: _vMono, activeTrackColor: accent, onChanged: (v) => _setMono(v)),
                      ),
                    ],
                  ),
                ),
                ...
                Center(
                  child: TextButton.icon(
                    ...
                    label: Text(l.resetAll, style: TextStyle(color: cs.onSurfaceVariant, fontSize: 13)),
                  ),
                ),
...
class _EQBandSlider extends StatelessWidget {
  ...
  const _EQBandSlider({required this.frequency, required this.level, required this.minLevel, required this.maxLevel, required this.accent, required this.enabled, required this.onChanged});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Text(
          '${level >= 0 ? "+" : ""}${level.toStringAsFixed(0)}',
          style: TextStyle(color: enabled ? cs.onSurfaceVariant : cs.onSurface.withValues(alpha: 0.2), fontSize: 10, fontWeight: FontWeight.w600),
        ),
        const SizedBox(height: 4),
        Expanded(
          child: RotatedBox(
            ...
            child: SliderTheme(
              ...
              child: Slider(value: level, min: minLevel, max: maxLevel, onChanged: enabled ? onChanged : null),
            ),
          ),
        ),
        const SizedBox(height: 4),
        Text(
          label,
          style: TextStyle(color: enabled ? cs.onSurfaceVariant : cs.onSurface.withValues(alpha: 0.15), fontSize: 10, fontWeight: FontWeight.w500),
        ),
        Text(
          eq.freqName(frequency),
          style: TextStyle(color: enabled ? cs.onSurfaceVariant.withValues(alpha: 0.5) : cs.onSurface.withValues(alpha: 0.1), fontSize: 9, fontWeight: FontWeight.w400),
        ),
      ],
    );
  }
...
class _EffectRow extends StatelessWidget {
  ...
  const _EffectRow({required this.icon, required this.label, required this.value, required this.accent, required this.enabled, required this.onChanged});
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            ...
            child: Text(
              label,
              style: TextStyle(color: enabled ? cs.onSurface.withValues(alpha: 0.7) : cs.onSurface.withValues(alpha: 0.24), fontSize: 12, fontWeight: FontWeight.w500),
            ),
          ),
          Expanded(
            child: SliderTheme(
              ...
              child: Slider(value: value, min: 0.0, max: 1.0, onChanged: enabled ? onChanged : null),
            ),
          ),
          SizedBox(
            width: 32,
            child: Text(
              ...
              style: TextStyle(color: enabled ? cs.onSurfaceVariant : cs.onSurface.withValues(alpha: 0.15), fontSize: 11, fontWeight: FontWeight.w600),
            ),
          ),
        ],
      ),

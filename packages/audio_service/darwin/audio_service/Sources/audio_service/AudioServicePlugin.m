...
- (void) updateNowPlayingInfo {
    ...
    // playbackState is available on iOS 13+ and macOS 10.12.2+. Upstream 0.18.18
    // wraps this in `#if TARGET_OS_OSX`, leaving it unset on iOS — and without
    // an explicit .paused signal, iOS dismisses the now-playing entry (and
    // Dynamic Island) a few seconds after pause, then suspends the app. Setting
    // it on iOS too keeps the entry persistent like Apple Podcasts.

...
static void android_whisper_log(enum ggml_log_level level, const char * text, void * user_data) {
    (void) user_data;
...
json transcribe(json jsonBody)
{
    ...
    // .value() throws on an explicit JSON null, which is what the Dart side
    // sends when VAD is disabled.
    ...
    whisper_log_set(android_whisper_log, nullptr);
    ...
    // Totals with per-op run counts go to logcat (tag WhisperCpp); the JSON
    // carries the per-run averages plus the wall total.
    whisper_print_timings(g_ctx);
    {
        json timing;
        timing["total_ms"] = (int64_t) duration;
        whisper_timings * wt = whisper_get_timings(g_ctx);
        if (wt != nullptr) {
            timing["encode_avg_ms"] = wt->encode_ms;
            timing["decode_avg_ms"] = wt->decode_ms;
            timing["sample_avg_ms"] = wt->sample_ms;
            timing["batchd_avg_ms"] = wt->batchd_ms;
            timing["prompt_avg_ms"] = wt->prompt_ms;
            delete wt;
        }
        jsonResult["timing"] = timing;
    }
    ...
    // When the request said "auto" this is what detection settled on, so the
    // caller can pass it explicitly next time and skip the detection encode.
    const char * lang = whisper_lang_str(whisper_full_lang_id(g_ctx));
    if (lang != nullptr) {
